...
plot_dict = {'tsnr': ('#D2691E'), 'gcor': ('#D2691E'), 'dvars_vstd': ('#D2691E'), 'dvars_std': ('#D2691E'), # temporal
            ...
            'fd_mean': ('#66CDAA'), 'fd_num': ('#00008B'), 'fd_perc': ('#A52A2A'), # motion IQMs
...
def chart(df, var_name, var_label):

    df_long = pd.melt(df, id_vars=['bids_name','SOURCE'],var_name='var',value_name='values')
    ...
    fig.update_layout(autosize=True,
                    ...
                    xaxis=go.layout.XAxis(title = go.layout.xaxis.Title(text=definition,font=dict(size=8))))
